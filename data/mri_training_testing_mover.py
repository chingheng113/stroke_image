...
if __name__ == '__main__':
    ...
    for index, row in id_train.iterrows():
        ...
        shutil.copy(os.path.join(source_path, 'n4_dwi_norm', train_id+'_DWI.nii'), training_dwi_path)
        shutil.copy(os.path.join(source_path, 'n4_flair_norm', train_id + '_FLAIR.nii'), training_flair_path)
    # testinf data
    for index, row in id_test.iterrows():
        test_id = str(row['MRI HEME #'])
        shutil.copy(os.path.join(source_path, 'n4_dwi_norm', test_id + '_DWI.nii'), testing_dwi_path)
        shutil.copy(os.path.join(source_path, 'n4_flair_norm', test_id + '_FLAIR.nii'), testing_flair_path)
